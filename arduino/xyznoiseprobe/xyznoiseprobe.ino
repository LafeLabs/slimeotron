// xyznoiseprobe
// take voltage from A0 input, which is from 0 to 255 multiply by some gain factor to get noise level
// create a randomx randomy and randomz which are all 
...
void loop() { 
  ...
  randomx = random(noise);
  randomy = random(noise);
  randomz = random(noise); 
  if(forward){
